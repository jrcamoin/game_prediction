...
from app.trained_model import trained_artifact_family
...
def model_status() -> ModelStatus:
    ...
    family = trained_artifact_family()
    active_model = f"Trained {family} stack" if family else "Deterministic stacked tree scaffold"

    return ModelStatus(
        active_model=active_model,
        trained_artifact_path=artifact_path,
        trained_artifact_loaded=artifact_loaded,
        capabilities=[_sport_capability(sport, bool(family)) for sport in Sport],
    )
...
def _sport_capability(sport: Sport, artifact_loaded: bool) -> SportCapability:
    ...
    return SportCapability(
        ...
        model_name="Trained tree stack" if artifact_loaded else "Stacked tree scaffold",
        notes=notes,
    )
